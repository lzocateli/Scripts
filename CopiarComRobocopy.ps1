...
$robocopycmd = "robocopy ""$source"" ""$destination"" ""$arquivo""  /mir /bytes" 

$Staging = Invoke-Expression "$robocopycmd /l" 
$totalnewfiles = $Staging -match 'Novo Arquivo' 
$totalmodified = $Staging -match 'Mais Recente' 
$totalfiles = $totalnewfiles + $totalmodified
$TotalBytesarray = @()
foreach ($file in $totalfiles)  
{ 
    if ($file.substring(17,13).trim().length -eq 9) {$TotalBytesarray+= $file.substring(17,13).trim() } 
    else {$TotalBytesarray+= $file.substring(17,13).trim()} 
} 
...
{ 
	...
	if ($progress)
	{
		$copiedfiles = ($progress | Select-String -SimpleMatch 'Novo Arquivo', 'Mais Recente')
		...
		{
			if ($Newfile.tostring().substring(17, 13).trim().length -eq 9) { $Bytesarray += $Newfile.tostring().substring(17, 13).trim() }
			else { $Bytesarray += $Newfile.tostring().substring(17, 13).trim() }
		}
		...
		if ($copiedfiles)
		{
			if ($copiedfiles[-1].tostring().substring(17, 13).trim().length -eq 9) { $currentfile = $copiedfiles[-1].tostring().substring(28).trim() }
